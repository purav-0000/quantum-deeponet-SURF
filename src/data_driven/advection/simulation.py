...
for x_branch0 in x_test[0]:
    x_branch = x_branch0
    x_branch = silu(tomo_output(20+1,21, x_branch, branch_hidden0_thetas,simulator)+branch_hidden0_bias)
    
    x_branch = silu(tomo_output(21,21, x_branch, branch_hidden1_thetas,simulator)+branch_hidden1_bias) + \
        x_branch/np.linalg.norm(x_branch)
    
    x_branch = silu(tomo_output(21,21, x_branch, branch_hidden2_thetas,simulator)+branch_hidden2_bias) + \
        x_branch/np.linalg.norm(x_branch)

    x_branch = silu(tomo_output(21,21, x_branch, branch_hidden3_thetas,simulator)+branch_hidden3_bias) + \
        x_branch/np.linalg.norm(x_branch)
    
    x_branch = silu(tomo_output(21,21, x_branch, branch_hidden4_thetas,simulator)+branch_hidden4_bias) + \
        x_branch/np.linalg.norm(x_branch)
...
for x_trunk in x_test[1]:
    x_trunk = silu(tomo_output(3, 21, x_trunk, trunk_hidden0_thetas,simulator)+trunk_hidden0_bias)
    
    x_trunk = silu(tomo_output(21,21, x_trunk, trunk_hidden1_thetas,simulator)+trunk_hidden1_bias) + \
        x_trunk/np.linalg.norm(x_trunk)
    
    x_trunk = silu(tomo_output(21,21, x_trunk, trunk_hidden2_thetas,simulator)+trunk_hidden2_bias) + \
        x_trunk/np.linalg.norm(x_trunk)
    
    x_trunk = silu(tomo_output(21,21, x_trunk, trunk_hidden3_thetas,simulator)+trunk_hidden3_bias) + \
        x_trunk/np.linalg.norm(x_trunk)
    
    x_trunk = silu(tomo_output(21,21, x_trunk, trunk_hidden4_thetas,simulator)+trunk_hidden4_bias) + \
        x_trunk/np.linalg.norm(x_trunk)
