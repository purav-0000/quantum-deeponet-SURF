...
for i in range(5):
    l2_errors,y_arrays = [],[]
    for shots in [1000,10000,100000,1000000,10000000,100000000,1000000000]:
        y_array = []
        for x in x_array:
            y = simple_function_NN(x,shots)
            y_array.append(y)
        l2_error = np.linalg.norm(y_array-y_true)/np.linalg.norm(y_true)
        l2_errors.append(l2_error)
        y_arrays.append(y_array)
    np.save(f'shots_based_y{i}.npy',y_arrays)
    np.savetxt(f'shots_based_error{i}.txt',l2_errors)
