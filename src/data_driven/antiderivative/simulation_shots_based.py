...
def process_state(idx, results, shots, n_in, n_out, hidden0_bias, output_weight, output_bias, trunk=False, final_layer=False):
...
def run_quantum_layer(inputs, n_in, n_out, shots, hidden0_bias, output_weight, output_bias, thetas, simulator, batch_size=None, trunk=False, final_layer=False, n_jobs=1):
    ...
    if batch_size is None:
        ...
        return np.array(Parallel(n_jobs=n_jobs)(
            delayed(process_state)(i, results, shots, n_in, n_out,
        ...
        for i in tqdm(range(0, len(inputs), batch_size), desc="Running in batches"):
            ...
            outputs.extend(Parallel(n_jobs=n_jobs)(
                delayed(process_state)(j, results, shots, n_in, n_out,
                                       hidden0_bias, output_weight, output_bias, trunk, final_layer)
...
def evaluate_model(branch_out, trunk_out, b, y_true, seed, run):
    x = (np.einsum('bi,ni->bn', branch_out, trunk_out) + b)
    mean_l2_error = np.mean(np.linalg.norm(x - y_true, axis=1) / np.linalg.norm(y_true, axis=1))
    output_dir = os.path.dirname(os.path.abspath(__file__))

    np.savetxt(os.path.join(output_dir, f'simulation_error_seed{seed}_run{run}.txt'), [mean_l2_error])
    np.savetxt(os.path.join(output_dir, f'simulation_output_seed{seed}_run{run}.txt'), x)
...
def main():
    ...
    for i in range(1):
        ...
        evaluate_model(branch_outputs, trunk_outputs, weights["b"], y_test, SEED, i)
